...
import com.example.bandungmentalhealthv10.databinding.FragmentJadwalBinding
...
class JadwalFragment : Fragment(R.layout.fragment_jadwal) {
    ...
    private fun readAllData() {
        val database: DatabaseReference =
            FirebaseDatabase.getInstance().getReference("Jadwal Konsultasi")
        database.get().addOnSuccessListener { dataSnapshot ->
            if (dataSnapshot.exists()) {
                ...
                Toast.makeText(
                    requireContext(),
                    "Anda Belum Memiliki Sesi Konsultasi",
                    Toast.LENGTH_LONG
                ).show()
            }
