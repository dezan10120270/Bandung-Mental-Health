...
import com.example.bandungmentalhealthv10.databinding.ActivityCreatePostBinding
...
class CreatePostActivity : AppCompatActivity(), SelectMoodBottomSheetFragment.SelectedMoodListener {
    ...
    private val startForPostImageResult =
    ...
    private fun inputValidation(): Boolean {
        ...
        if (postType == "Diary" && selectedMood == null) {
            isValid = false
            Snackbar.make(binding.root, "Anda belum memilih suasana hati", Snackbar.LENGTH_SHORT)
                .show()
            binding.btPublish.isClickable = true
        }
        ...
        if (binding.etCaption.text.toString().trim().isEmpty()) {
            isValid = false
            binding.etCaption.error = "Caption tidak boleh kosong!"
            binding.btPublish.isClickable = true
        }
    ...
    private fun updateUI(type: String) {
        if (type == "Diary") {
            ...
            if (type == "Public") binding.tvTitleToolbar.text =
                getString(R.string.title_create_post_public)
