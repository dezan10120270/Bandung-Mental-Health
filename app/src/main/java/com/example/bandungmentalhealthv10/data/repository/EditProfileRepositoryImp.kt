...
import com.example.bandungmentalhealthv10.utils.FirebaseStorageConstants
...
class EditProfileRepositoryImp(
    ...
    override fun deleteProfilePicture(imageUrl: String, result: (UiState<String>) -> Unit) {
        storageReference.getReferenceFromUrl(imageUrl).delete()
            .addOnSuccessListener {
                result.invoke(
                    UiState.Success("Foto profil telah dihapus")
                )
            }
    ...
    override fun updateUserProfile(
        ...
        database.collection(FirestoreCollection.USER)
            ...
            .addOnSuccessListener {
                result.invoke(
                    UiState.Success("Data pengguna telah diperbarui")
                )
            }
            .addOnFailureListener {
                result.invoke(
    ...
    private fun updateAllUserCommentsData(
        newUserData: UserData,
        result: (UiState<String>) -> Unit
    ) {
        database.collection(FirestoreCollection.COMMENT)
            ...
            .addOnSuccessListener {
                ...
                for (document in it) {
                    ...
                    batch.update(
                        document.reference,
                        FirestoreDocumentField.COMMENT_PROFILE_PICTURE,
                        newUserData.profilePicture,
                        FirestoreDocumentField.COMMENT_USER_NAME,
                ...
                batch.commit()
                    .addOnSuccessListener {
                        result.invoke(
                            UiState.Success("Komentar berhasil diubah")
                        )
                    }
                    .addOnFailureListener {
                        result.invoke(
        ...
        database.collection(FirestoreCollection.POST)
            ...
            .addOnSuccessListener {
                ...
                for (document in it) {
                    ...
                    batch.update(
                        document.reference,
                        FirestoreDocumentField.POST_PROFILE_PICTURE,
                        newUserData.profilePicture,
                        FirestoreDocumentField.POST_USER_NAME,
                        newUserData.userName
                    )
                }
                batch.commit()
                    .addOnSuccessListener {
                        result.invoke(
                            UiState.Success("Postingan berhasil diubah")
                        )
                    }
                    .addOnFailureListener {
                        result.invoke(
