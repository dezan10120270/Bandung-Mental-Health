...
import com.example.bandungmentalhealthv10.data.model.FollowData
...
class PostRepositoryImp(
    ...
    override fun getUserPosts(
        type: String,
        uid: String,
        result: (UiState<List<PostData>>) -> Unit
    ) {
    ...
    override fun addPost(post: PostData, result: (UiState<String>) -> Unit) {
        ...
        document.set(post)
            .addOnSuccessListener {
                result.invoke(
                    UiState.Success("Postingan telah dipublikasikan")
                )
            }
            .addOnFailureListener {
                result.invoke(
    ...
    override fun addDiary(diary: DiaryData, result: (UiState<String>) -> Unit) {
        ...
        document.set(diary)
            .addOnSuccessListener {
                result.invoke(
                    UiState.Success("Diary telah dipublikasikan")
                )
            }
            .addOnFailureListener {
                result.invoke(
    ...
    override fun addComment(comment: CommentData, result: (UiState<String>) -> Unit) {
        ...
        database.runTransaction { transaction ->
            ...
            .addOnSuccessListener {
                result.invoke(
                    UiState.Success("Komentar telah dipublikasikan")
                )
            }
            .addOnFailureListener {
                result.invoke(
        ...
        documentLike.get().addOnSuccessListener {
            if (!it.exists()) {
                addLikeDataToUser(like, postId, currentUserId).apply {
                    val likesAmount =
                        getPost.result.get(FirestoreDocumentField.LIKES_COUNT).toString().toLong()
                            .plus(1)
